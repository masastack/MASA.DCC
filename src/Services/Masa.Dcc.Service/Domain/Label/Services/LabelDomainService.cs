...
public class LabelDomainService : DomainService
{
    ...
    public async Task AddLabelAsync(UpdateLabelDto labelDto)
    {
        ...
        else
        {
            ...
            List<Dcc.Infrastructure.Domain.Label.Aggregates.Label> labels = new();
            foreach (var item in labelDto.LabelValues)
            {
                var labelEntity = new Dcc.Infrastructure.Domain.Label.Aggregates.Label(item.Code,
                    item.Name,
                    labelDto.TypeCode,
                    labelDto.TypeName,
    ...
    public async Task UpdateLabelAsync(UpdateLabelDto labelDto)
    {
        ...
        if (labelEntities.Any())
        {
            ...
            foreach (var item in labelDto.LabelValues)
            {
                labels.Add(new Dcc.Infrastructure.Domain.Label.Aggregates.Label(item.Code,
                    item.Name,
                    labelDto.TypeCode,
                    labelDto.TypeName,
                    labelEntity.Creator,
